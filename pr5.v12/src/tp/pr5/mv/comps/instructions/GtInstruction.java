...
public class GtInstruction extends Comparaciones {
	...
	public boolean operar(OperandStack op) throws ExceptionStack {
		...
		if(op.sizePila() >= 2){
			...
			ok = false;
			throw new ExceptionStack("GT", op.sizePila());
		}
		return ok;
	...
	public Instruction parse(String line) throws NumberFormatException {
		...
		if(subCadenas.length == 1 && subCadenas[0].equalsIgnoreCase("GT")){
			inst = new GtInstruction();
		}
		return inst;
	}
